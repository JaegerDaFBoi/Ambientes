...
class Card extends Model
{
  ...
  {
    try {
      $sql = $this->connection->query("SELECT fichas.id, fichas.numero, programas.nombre AS programaformacion, fichas.jornada, fichas.modalidad, fichas.fechainicio, fichas.fechafin, instructores.nombre AS nombreinstructor, fichas.cantidad FROM fichas INNER JOIN programas ON fichas.idprograma = programas.id INNER JOIN instructores ON fichas.idinstructor = instructores.id WHERE fichas.id=".$_GET['id']);
  ...
  {
    try {
      $sql = "UPDATE fichas SET numero=:numero, idprograma=:idprograma, jornada=:jornada, modalidad=:modalidad, fechainicio=:fechainicio, fechafin=:fechafin, idinstructor=:idinstructor, cantidad=:cantidad WHERE id=:id";
      $query = $this->connection->prepare($sql);
